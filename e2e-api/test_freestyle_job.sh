...
curl -X POST -H "Authorization: Bearer ${TEST_USER_TOKEN}" -H "Content-Type: application/json" --data '{"name": "manual_freestyle_job_name", "dockerImage": "busybox:1.33", "command": ["sh", "-c", "echo \"Hello world\""]}' $JOB_HOST/jobs
...
curl -X POST -H "Content-Type: application/json" --data '{"name": "manual_freestyle_job_name", "dockerImage": "busybox:1.33", "command": ["sh", "-c", "echo \"Hello world\""]}' $JOB_HOST/jobs
...
### With input:
curl -X POST -H "Content-Type: application/json" --data '{"name": "manual_freestyle_job_name", "dockerImage": "busybox:1.33", "command": ["sh", "-c", "expr $MINUEND - $SUBTRAHEND"]}' $JOB_HOST/jobs
